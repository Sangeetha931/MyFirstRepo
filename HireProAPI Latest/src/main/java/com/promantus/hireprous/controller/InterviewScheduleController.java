...
public class InterviewScheduleController extends CommonController {
	 ...
	 * @return
	 */
	@GetMapping("/getInterviewScheduleByCandiateIdAndRecStatus/{candiateId}/{recStatus}")
	public InterviewScheduleDto getInterviewScheduleByCandiateIdAndRecStatus(@PathVariable String candiateId,
			@PathVariable String recStatus, @RequestHeader(name = "lang", required = false) String lang) {

		InterviewScheduleDto interviewScheduleDto = new InterviewScheduleDto();
		try {
			interviewScheduleDto = interviewScheduleService.getInterviewScheduleByCandidateIdAndRecStatus(candiateId, recStatus);
		} catch (final Exception e) {
			logger.error(HireProUsUtil.getErrorMessage(e));
		}
