...
//hold to passed/rejected stats//
  holdedInterviewschedule: interviewSchedule = new interviewSchedule();
  getHoldedStatus(item){
    this.candidateDto = item;
    console.log(item);
    this.rserv.getInterviewScheduleByCandiateIdAndRecStatus(this.candidateDto.id, this.candidateDto.recStatus ).subscribe((data)=>{
      console.log(data);
      this.holdedInterviewschedule = data;
    });
  }
  interviewSchedule : interviewSchedule = new interviewSchedule();
  holdToOtherStatus(f:NgForm){
    console.log(f);
    if (f.form.valid) {
      if (this.interviewSchedule.status == '1') {
        switch (this.candidateDto.recStatus){
          case "06":{
      ...
      this.interviewSchedule.id = this.holdedInterviewschedule.id;
      this.interviewSchedule.updatedBy = this.loggedInUserId;
      this.interviewSchedule.createdBy = this.holdedInterviewschedule.createdBy;
      this.interviewSchedule.round = this.holdedInterviewschedule.round
  ...
  resumeHoldToOtherStatus(f:NgForm){
